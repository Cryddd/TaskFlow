...
export function getTagSuggestionGroups({
  ...
  const categoryItems = (CATEGORY_TAGS[category] ?? CATEGORY_TAGS.work).map((item, i) => ({
    ...item,
    score: 80 - i,
  }));
  groups.push({
  ...
  const topSuggestions = uniqueByTag(flat, 14);
...
export function getSubtaskSuggestionGroups({
  ...
  const matchedTemplates = templates.filter((t) => t.score > 0).slice(0, 3);
  const fallbackTemplates = templates.filter((t) => t.score === 0).slice(0, 2);
  const templateList = [...matchedTemplates, ...fallbackTemplates].slice(0, 4);

  const fromHistory = collectSubtaskTitles(tasks, { category, tags })
  ...
  const contextualSingles = [
    ...(DIFFICULTY_SUBTASKS[difficulty] ?? []).map((title, i) => ({
      title,
      reason: `${difficulty} task`,
      score: 40 - i,
    })),
    ...(PRIORITY_SUBTASKS[priority] ?? []).map((title, i) => ({
      title,
