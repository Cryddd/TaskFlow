...
export async function fetchTaskById(id) {
  ...
  const task = mapTask(data, subtasksMap[id] ?? []);
  task.tags = tagsMap[id] ?? [];
  return task;
}
...
export async function createTask(task) {
  ...
  if (subtasks.length) {
    const { error: subtaskErr } = await supabase.from('subtasks').insert(
      subtasks.map((s, i) => ({
        task_id: data.id,
        user_id: user.id,
        title: s.title,
  ...
  if (tags.length) {
    await syncTaskTags(data.id, user.id, tags);
  }

  return fetchTaskById(data.id);
}

async function syncTaskTags(taskId, userId, tags) {
  const { error: deleteErr } = await supabase.from('task_tags').delete().eq('task_id', taskId);
  if (deleteErr) throw deleteErr;

  for (const tag of tags) {
    const name = tag.replace(/^#/, '');
    const { data: tagRow, error: tagLookupErr } = await supabase
      .from('tags')
      .select('id')
      .eq('user_id', userId)
      .eq('name', name)
    ...
    const { error: linkErr } = await supabase
      .from('task_tags')
      .insert({ task_id: taskId, tag_id: resolvedTag.id });
    if (linkErr) throw linkErr;
  }
}
...
export async function updateTask(id, updates) {
  ...
  if (subtasks !== undefined) {
    const { error: deleteSubtaskErr } = await supabase.from('subtasks').delete().eq('task_id', id);
    if (deleteSubtaskErr) throw deleteSubtaskErr;
    if (subtasks.length) {
      const { error: subtaskErr } = await supabase.from('subtasks').insert(
        subtasks.map((s, i) => ({
          task_id: id,
  ...
  if (tags !== undefined) {
    await syncTaskTags(id, user.id, tags);
  }
